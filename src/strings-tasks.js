...
function removeFirstOccurrences(str, value) {
  ...
  if (firstIndex !== -1) {
    const start = str.substring(0, firstIndex);
    const end = str.substring(firstIndex + value.length);
    return start + end;
  }
...
function removeLastOccurrences(str, value) {
  ...
  if (lastIndex !== -1) {
    const start = str.substring(0, lastIndex);
    const end = str.substring(lastIndex + value.length);
    return start + end;
  }
...
function countVowels(str) {
  ...
  const result = str.split('').reduce((sum, item) => {
...
function isPalindrome(str) {
  const newString = str.toLowerCase().replace(/[,?!]/g, '').replaceAll(' ', '');
...
function extractNameFromTemplate(value) {
  return value.replace('!', '').split(', ').slice(1, 2).join('');
}
